...
export default function CreateMilestoneModal({
  ...
  const allocatedAmount = existingMilestones.reduce(
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.message || "Unable to create milestone."
        );
      }

      onMilestoneCreated(data.milestone);
      onClose();
    } catch (error) {
      console.error("CREATE_MILESTONE_ERROR", error);
        ...
        <div className="flex items-start justify-between gap-5">
          <div>
            ...
            <p className="mt-2 text-sm text-[#B88746]">
              Divide the project into clear deliverables and payment stages.
            </p>
          </div>
